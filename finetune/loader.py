...
def mol_to_graph_data_obj_grover(mol):
    ...
    f_atoms = torch.tensor(f_atoms)
    bond_list = torch.tensor(np.array(bond_list).T, dtype=torch.long)
    f_bonds = torch.tensor(np.array(f_bonds), dtype=torch.float)
