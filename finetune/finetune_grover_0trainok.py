# python finetune_grover.py --dataset tox21 --epochs 100 --output_path output_grover/tox21_RS1 --batch_size 96 > grover_tox21_RS1.txt &
...
def run_training(args: Namespace, logger: Logger = None):
    ...
    #set up dataset
    dataset = MoleculeDataset_grover("dataset_grover/" + args.dataset, dataset=args.dataset)
    ...
    #set up model
    model = GNN_graphpred_grover(args.num_layer, args.emb_dim, num_tasks, JK = args.JK, drop_ratio = args.dropout_ratio, graph_pooling = args.graph_pooling, gnn_type = args.gnn_type)
    #if not args.input_model_file == "":
        #model.from_pretrained(args.input_model_file) #dont need because non-pretrained
