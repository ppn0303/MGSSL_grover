    ...
    for step, batch in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            if args.multi_class and not args.regression:
                ...
                pred = torch.tensor(pred, dtype=torch.float).to(device)
                y = batch.y.view(pred.shape).to(torch.float)
            else : 
                y = batch.y.view(pred.shape).to(torch.float64)
            ...
            if args.dataset=='qm7' or args.dataset=='qm8':
                ...
                loss = math.sqrt(mean_squared_error(y.cpu().detach().numpy(), pred.cpu().detach().numpy()))
                loss = np.mean(loss)
            else:
                loss = calcul_loss(pred, y, args)
...
def run_training(args: Namespace, logger: Logger = None):
    ...
    if not args.regression:
        test_loss, auc, acc, rec, prec, f1s, tp, fp, tn, fn = test(args, model, device, test_loader)
