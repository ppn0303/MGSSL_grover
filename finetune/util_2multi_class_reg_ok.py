...
from loader import MoleculeDataset_other
...
if __name__ == "__main__":
    ...
    dataset = MoleculeDataset_other("dataset/tox21", dataset="tox21")
    transform(dataset[0])
...
def calcul_loss(pred, true, args):
    if args.regression:
        ...
        if args.multi_class : 
            ...
            is_valid = true**2 > 0
            #Loss matrix
            loss_mat = loss_func(pred.double(), (true+1)/2)
...
def confusion_mat(targets: List[int], preds: List[float], args: Namespace, threshold: float = 0.5) -> float:
    """
    ...
    if args.multi_class:
        ...
        #BA = (rec+spe)/2
    return auc, acc, rec, prec, f1s, tp, fp, tn, fn
