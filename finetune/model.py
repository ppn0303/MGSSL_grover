...
class GNN_graphpred(torch.nn.Module):
    ...
    def from_pretrained(self, model_file):
        ...
        try : self.gnn.load_state_dict(torch.load(model_file))
        except : 
            cp = torch.load(model_file)["MODEL_STATE"]
...
######## add for grover feature trainging
...
class GATConv_grover(MessagePassing):
    def __init__(self, emb_dim, heads=2, negative_slope=0.2, aggr = "add"):
        super(GATConv_grover, self).__init__()
        ...
        self.weight_linear = torch.nn.Linear(emb_dim, heads * emb_dim)
        self.att = torch.nn.Parameter(torch.Tensor(1, heads, bond_fdim * emb_dim))

        self.bias = torch.nn.Parameter(torch.Tensor(emb_dim))

        self.edge_embedding = torch.nn.Linear(bond_fdim, heads * emb_dim)
    ...
    def forward(self, x, edge_index, edge_attr):
        print(f'edge_index shape : {edge_index.shape}, {x.size(0)}')
        #add self loops in the edge space
        edge_index = add_self_loops(edge_index, num_nodes = x.size(0))
        print(f'edge_index shape : {len(edge_index)}')
        norm = self.norm(edge_index, x.size(0), x.dtype)
        print(norm.shape)
...
class GraphSAGEConv_grover(MessagePassing):
    def __init__(self, emb_dim, aggr = "mean"):
        super(GraphSAGEConv_grover, self).__init__()
