...
def train(args, model_list, loader, optimizer_list, epoch, best_val_loss, resume_batch=0):
    ...
    for step, batch in enumerate(loader):	# 데이터로더에서 순회 진행바 표시형태로 순회해서 step과 batch대로 반복하자
        ...
        if pass_ok : 
            ...
            graph_batch = graph_batch.to(args.gpu_id)			# 그래프배치는 GPU로 되게
...
def main(args):
    ...
    #for distributed
    args.global_rank = int(os.environ["RANK"]) if args.multi else 0
    args.local_rank = int(os.environ["LOCAL_RANK"]) if args.multi else 0
    args.world_size = int(os.environ["WORLD_SIZE"]) if args.multi else 1
    ## binding training to GPUs.
    args.master_worker = (global_rank == 0) if args.multi else True
    if 'SLURM_PROCID' in os.environ:
        args.global_rank = int(os.environ['SLURM_PROCID'])
        args.gpu_id = args.global_rank % torch.cuda.device_count()
    
    init_process_group(backend="nccl", init_method='env://', world_size = args.world_size, rank = args.global_rank)
    ...
    info(f'global_rank : {args.global_rank}, local_rank : {args.local_rank}, gpu_id : {args.gpu_id}, world_size : {args.world_size}')
    ...
    if args.grover_dataset:
        ...
        pre_load_data(dataset=train_dataset, rank=args.local_rank, num_replicas=args.world_size, sample_per_file=sample_per_file, logger=logger)
        pre_load_data(dataset=val_dataset, rank=args.local_rank, num_replicas=args.world_size, sample_per_file=sample_per_file, logger=logger)
        
        train_sampler = DistributedSampler(dataset=train_dataset, num_replicas=args.world_size, rank=args.local_rank, shuffle=True, sample_per_file=sample_per_file)
        val_sampler = DistributedSampler(dataset=val_dataset, num_replicas=args.world_size, rank=args.local_rank, shuffle=False, sample_per_file=sample_per_file)
    ...
    model = GNN(5, args.emb_dim, JK='last', drop_ratio=args.dropout_ratio, gnn_type='gin').cuda(args.gpu_id)
    model = DDP(model, device_ids = [args.gpu_id])

    vocab = [x.strip("\r\n ") for x in open(args.vocab)]
    vocab = Vocab(vocab)
    motif_model = Motif_Generation_Grover(vocab, args.hidden_size, args.gpu_id, args.order).cuda(args.gpu_id)
    motif_model = DDP(motif_model, device_ids = [args.gpu_id])
