#reference : https://github.com/pytorch/examples/blob/main/distributed/ddp/main.py   (23.8.16)
...
def parse_args():
    ...
    parser.add_argument('--master_addr', type=str, default="", help="what is the master_addr")
    parser.add_argument('--master_port', type=str, default='1138', help="what is the master_port")
...
def main(local_world_size, local_rank, args):
    ...
    #args = parse_args()
    #for distributed
    args.global_rank = dist.get_rank() if args.multi else 0
    #args.local_rank = args.global_rank % torch.cuda.device_count()
    args.world_size = int(os.environ["WORLD_SIZE"]) if args.multi else 1
    args.master_worker = (args.global_rank == 0) if args.multi else True

    time1 = time.time()

    info(f'global_rank : {args.global_rank}, local_rank : {args.rank}, rank : {rank}, world_size : {args.world_size}')
