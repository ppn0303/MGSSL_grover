...
def train(args, model_list, loader, optimizer_list, epoch, best_val_loss, resume_batch=0):
    ...
    for step, batch in enumerate(loader):	# 데이터로더에서 순회 진행바 표시형태로 순회해서 step과 batch대로 반복하자
        ...
        if pass_ok : 
            ...
            graph_batch = graph_batch.to(args.local_rank)
...
def main(args):
    ...
    model = GNN(5, args.emb_dim, JK='last', drop_ratio=args.dropout_ratio, gnn_type='gin').to(args.local_rank)
    model = DDP(model, device_ids = [args.local_rank])

    vocab = [x.strip("\r\n ") for x in open(args.vocab)]
    vocab = Vocab(vocab)
    motif_model = Motif_Generation_Grover(vocab, args.hidden_size, args.local_rank, args.order).to(args.local_rank)
    motif_model = DDP(motif_model, device_ids = [args.local_rank])
