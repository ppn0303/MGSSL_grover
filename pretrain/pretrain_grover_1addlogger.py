#python pretrain_grover.py --emb_dim 1200 --hidden_size 1200 --epochs 100 --dropout_ratio 0.1 --dataset data/merge_0/moltrees/0.p --vocab data/merge_0/clique.txt --output_path saved_model/grover --batch_size 40 --order dfs
#python pretrain_grover.py --emb_dim 300 --hidden_size 300 --epochs 100 --dropout_ratio 0.1 --dataset data/merge_0 --vocab data/merge_0/clique.txt --output_path saved_model/grover --batch_size 40 --order dfs --grover_dataset
import argparse
import torch
...
def main():
    ...
    if args.grover_dataset:
        ...
        train_dataset, val_dataset, _ = split_data_grover(data=grover_data, sizes=(0.8,0.2,0), seed=args.seed, logger=logger)
        shared_dict = {}
        GMC = GroverMotifCollator(shared_dict=shared_dict, args=args)
        pre_load_data(train_dataset, rank = rank, num_replicas = num_replicas, logger=logger)
        pre_load_data(val_dataset, rank = rank, num_replicas = num_replicas, logger=logger)
