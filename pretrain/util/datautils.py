...
def moltree_to_grover_data(batch):
    graph_data_batch = []
    for mol in batch:
...
def save_cp(model, motif_model, optimizer, optimizer_motif, path, epoch, batch, val_loss, ddp=False):
    snapshot = {}
    if ddp:
        snapshot["MODEL_STATE"] = model.module.state_dict()
        snapshot["MOTIF_MODEL_STATE"] = motif_model.module.state_dict()
    else:
        snapshot["MODEL_STATE"] = model.state_dict()
        snapshot["MOTIF_MODEL_STATE"] = motif_model.state_dict()
    snapshot["optimizer_state"] = optimizer.state_dict()
    snapshot["optimizer_motif_state"] = optimizer_motif.state_dict()
    snapshot["EPOCHS_RUN"] = epoch
    snapshot["BATCH_RUN"] = batch
    snapshot["BEST_VAL_LOSS"] = val_loss
    torch.save(snapshot, path)
    print(f"Epoch {epoch}, Batch {batch} | Training cp saved")


def load_cp(model, motif_model, optimizer, optimizer_motif, path, ddp=False):
    snapshot = torch.load(path)
    if ddp:
        model.module.load_state_dict(snapshot["MODEL_STATE"])
        motif_model.module.load_state_dict(snapshot["MOTIF_MODEL_STATE"])
    else:
        model.load_state_dict(snapshot["MODEL_STATE"])
        motif_model.load_state_dict(snapshot["MOTIF_MODEL_STATE"])
    optimizer.load_state_dict(snapshot["optimizer_state"])
    optimizer_motif.load_state_dict(snapshot["optimizer_motif_state"])
    epochs_run = snapshot["EPOCHS_RUN"]
    batch_run = snapshot["BATCH_RUN"]
    val_loss = snapshot["BEST_VAL_LOSS"]
    print(f"Resuming training from snapshot at Epoch {epochs_run}")
    return epochs_run, batch_run, val_loss