...
def main(args):
    ...
    info(f'global_rank : {args.global_rank}, local_rank : {args.rank}, world_size : {args.world_size}')
    ...
    if args.grover_dataset:
        ...
        pre_load_data(dataset=train_dataset, rank=args.rank, num_replicas=args.world_size, sample_per_file=sample_per_file, logger=logger)
        pre_load_data(dataset=val_dataset, rank=args.rank, num_replicas=args.world_size, sample_per_file=sample_per_file, logger=logger)
        
        train_sampler = DistributedSampler(dataset=train_dataset, num_replicas=args.world_size, rank=args.rank, shuffle=True, sample_per_file=sample_per_file)
        val_sampler = DistributedSampler(dataset=val_dataset, num_replicas=args.world_size, rank=args.rank, shuffle=False, sample_per_file=sample_per_file)
