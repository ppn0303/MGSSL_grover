...
def train(args, model_list, loader, optimizer_list, epoch, best_val_loss, resume_batch=0):
    stime = time.time()
...
def validation(args, model_list, loader):
    ...
    for step, batch in enumerate(loader):	# 데이터로더에서 순회 진행바 표시형태로 순회해서 step과 batch대로 반복하자
        ...
        #store graph object data in the process stage	
        batch_index = graph_batch.batch.numpy()			# 배치내의 배치텐서를 넘파이로 인덱스에 넘겨라
        graph_batch = graph_batch.to(args.rank)			# 그래프배치는 GPU로 되게
        node_rep = model(graph_batch.x, graph_batch.edge_index, graph_batch.edge_attr)	# GNN모델에 그래프(x, 엣지인덱스, 엣지의 특성) 투입
        node_rep = group_node_rep(node_rep, batch_index, batch_size)			# rep는 representation의 줄임말로 노드 표현을 의미
        loss, word_loss, topo_loss, wacc, tacc = motif_model(batch, node_rep)		# motif모델에서 손실, motif정확도, 위상 정확도 출력
...
def main(args):
    ...
    #for distributed
    args.global_rank = hvd.rank() if args.multi else 0
    args.local_rank = hvd.local_rank() if args.multi else 0
    args.world_size = mgw.size() if args.multi else 1
    ## binding training to GPUs.
    args.master_worker = (hvd.rank() == 0) if args.multi else True
    ...
    info(f'global_rank : {args.global_rank}, local_rank : {args.local_rank}, world_size : {args.world_size}')
    ...
    time2 = time.time()
    debug(f'rank{args.global_rank} setup time is {time2-time1}')
    ...
    if args.grover_dataset:
        ...
        pre_load_data(dataset=train_dataset, rank=args.global_rank, num_replicas=args.world_size, sample_per_file=sample_per_file, logger=logger)
        pre_load_data(dataset=val_dataset, rank=args.global_rank, num_replicas=args.world_size, sample_per_file=sample_per_file, logger=logger)
        
        train_sampler = DistributedSampler(dataset=train_dataset, num_replicas=args.world_size, rank=args.global_rank, shuffle=True, sample_per_file=sample_per_file)
        val_sampler = DistributedSampler(dataset=val_dataset, num_replicas=args.world_size, rank=args.global_rank, shuffle=False, sample_per_file=sample_per_file)
    ...
    time3 = time.time()
    debug(f'rank{args.global_rank} data loading time is {time3-time2}')

    model = GNN(5, args.emb_dim, JK='last', drop_ratio=args.dropout_ratio, gnn_type='gin').cuda()

    vocab = [x.strip("\r\n ") for x in open(args.vocab)]
    vocab = Vocab(vocab)
    motif_model = Motif_Generation_Grover(vocab, args.hidden_size, args.local_rank, args.order).cuda()
    ...
    if os.path.exists(cp_path):
        ...
        if args.master_worker:
            ...
            debug(f'rank : {args.global_rank} load checkpoint : {resume_epoch}epoch')
    else : 
        resume_epoch = 0
        resume_batch = 0
        best_val_loss = 1e+10
    ...
    if args.multi : 
        ...
        hvd.broadcast_parameters(motif_model.state_dict(), root_rank=0)
        hvd.broadcast_optimizer_state(optimizer_model, root_rank=0)
        hvd.broadcast_optimizer_state(optimizer_motif, root_rank=0)
        optimizer_model = hvd.DistributedOptimizer(optimizer_model, named_parameters=model.named_parameters())
        optimizer_motif = hvd.DistributedOptimizer(optimizer_motif, named_parameters=motif_model.named_parameters())
    ...
    time4 = time.time()
    debug(f'rank{args.global_rank} model ready time is {time4-time3}')
    ...
    for epoch in range(1 + resume_epoch, args.epochs + 1):
        ...
        train_loss, train_node_loss, train_topo_loss, train_node_acc, train_topo_acc = train(args, model_list, train_loader, optimizer_list, epoch, best_val_loss, resume_batch=resume_batch)
        ...
        if args.master_worker :
            ...
            debug(f'rank {args.global_rank} epoch : {epoch:04d} train_loss : {train_loss:.4f} train_node_loss : {train_node_loss:.4f} train_topo_loss : {train_topo_loss:.4f} train_node_acc : {train_node_acc:.2f} train_topo_acc : {train_topo_acc:.2f} train_time : {train_end:.2f}s')
        ...
        if args.master_worker : 
            ...
            debug(f'rank {args.global_rank} epoch : {epoch:04d} val_loss : {val_loss:.4f} val_node_loss : {val_node_loss:.4f} val_topo_loss : {val_topo_loss:.4f} val_node_acc : {val_node_acc:.2f} val_topo_acc : {val_topo_acc:.2f} val_tim : {val_end:.2f}s')
        ...
        if args.master_worker:
            save_cp(model, motif_model, optimizer_model, optimizer_motif, os.path.join(args.output_path, 'temp.pth'), epoch, 0, best_val_loss)
            if best_val_loss > val_loss:
                save_cp(model, motif_model, optimizer_model, optimizer_motif, os.path.join(args.output_path, 'best.pth'), epoch, 0, best_val_loss)
            if epoch % 5 == 0:
                save_cp(model, motif_model, optimizer_model, optimizer_motif, os.path.join(args.output_path, f'{epoch}.pth'), epoch, 0, best_val_loss)
